...
export default function DataTable({ columns = [], rows = [] }) {
  ...
  return (
    <div className="w-full overflow-x-auto">
      <div className="text-sm text-gray-600 mb-4 flex flex-wrap items-center gap-4 bg-gray-50 p-3 rounded-lg">
        <div className="flex items-center gap-2">
          <span className="font-semibold">Sort:</span>
          <span className="text-gray-700">{sortKey ? `${sortKey} (${sortDir})` : 'none'}</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-700">{filteredAndSorted.length} stocks</span>
        </div>
      </div>
      ...
      <div className="rounded-lg overflow-hidden border border-gray-200 shadow-sm">
        <table className="min-w-full bg-white">
          <thead className="bg-gradient-to-r from-gray-100 to-gray-200">
            <tr>
              {columns.map(col => (
                <th key={col.key} className="px-4 py-3 text-left align-top border-b-2 border-gray-300">
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggleSort(col.key)} className="flex items-center gap-1 hover:text-blue-600 transition-colors">
                    ...
                    <div className="relative">
                      <button
                        ...
                        className={`ml-1 px-2 py-1 rounded transition-colors ${filters[col.key] ? 'bg-blue-500 text-white' : 'text-gray-400 hover:bg-gray-200'}`}
                        title="Filter"
                      >
                        ⚲
                      </button>
                      ...
                      {openFilter === col.key && (
                        <div className="absolute z-10 mt-2 p-4 bg-white border-2 border-blue-300 rounded-lg shadow-xl w-64">
                          <div className="flex justify-between items-center mb-3">
                            <strong className="text-sm text-gray-800">{col.label} filter</strong>
                            <button className="text-xs text-red-600 hover:text-red-700 font-medium" onClick={() => { clearFilter(col.key); setOpenFilter(null); }}>Clear</button>
                          </div>

                          {col.type === 'number' && (
                            <div className="flex flex-col gap-3">
                              <div>
                                <label className="text-xs text-gray-600 font-medium block mb-1">Min</label>
                                <input
                                  type="number"
                                  onChange={(e) => {
                                  ...
                                  className="border-2 border-gray-300 rounded-lg px-3 py-2 w-full focus:border-blue-500 focus:outline-none"
                                />
                              </div>
                              <div>
                                <label className="text-xs text-gray-600 font-medium block mb-1">Max</label>
                                <input
                                  type="number"
                                  onChange={(e) => {
                                    const current = filters[col.key] || {};
                          ...
                          {col.type === 'category' && (
                            <div className="max-h-48 overflow-auto">
                              { (uniqueValues[col.key] || []).map((val) => (
                                <label key={String(val)} className="flex items-center gap-2 text-sm py-1 hover:bg-gray-50 cursor-pointer">
                                  <input
                                    type="checkbox"
                          ...
                          {col.type === 'string' && (
                            <div>
                              <label className="text-xs text-gray-600 font-medium block mb-1">Contains</label>
                              <input
                                type="text"
                                onChange={(e) => {
                          ...
                          <div className="flex justify-end gap-2 mt-4">
                            <button onClick={() => setOpenFilter(null)} className="text-sm px-4 py-2 rounded-lg border-2 border-gray-300 hover:bg-gray-50 font-medium">Done</button>
                          </div>
                        </div>
                      )}
                    </div>
          ...
          <tbody>
            {filteredAndSorted.length === 0 && (
              <tr>
                <td className="p-8 text-center text-gray-500" colSpan={columns.length}>
            ...
            {filteredAndSorted.map((row, i) => (
              <tr key={row.symbol ?? i} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors`}>
                {columns.map(col => (
                  <td key={col.key} className="px-4 py-3 align-top border-b border-gray-200 text-sm">
                    {renderCell(row, col)}
                  </td>
                ))}
              </tr>
  ...
  function renderCell(row, col) {
    ...
    if (col.type === 'number') {
      ...
      if (col.key.includes('Pct') || col.key.includes('pct') || col.key.includes('Growth') || col.key.includes('Margin') || col.key.includes('Yield')) {
        if (v > 0) colorClass = 'text-green-600 font-semibold';
        else if (v < 0) colorClass = 'text-red-600 font-semibold';
      }
      
      // RSI - overbought/oversold
      if (col.key === 'rsi14') {
        if (v > 70) colorClass = 'text-red-600 font-semibold';
        else if (v < 30) colorClass = 'text-green-600 font-semibold';
      }
    ...
    if (col.key === 'marketStage') {
      ...
      const colorClass = stageColors[v] || 'bg-gray-100 text-gray-800';
      return (
    ...
    if (col.key === 'symbol') {
      return <span className="font-bold text-blue-600">{String(v)}</span>;
    }
