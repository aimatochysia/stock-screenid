...
export default function Tabs({ tabs = [], active, onChange }) {
  ...
  return (
    <div className={`flex gap-2 border-b-2 overflow-x-auto ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
      {tabs.map(t => (
        <button
          key={t}
          onClick={() => onChange(t)}
          className={`px-4 py-3 font-medium rounded-t-lg transition-all flex items-center gap-2 whitespace-nowrap flex-shrink-0
            ${active === t 
